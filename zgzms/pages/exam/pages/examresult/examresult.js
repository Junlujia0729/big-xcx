  ...
  onLoad(params) {
    ...
    userApi.xcx_login_new(1,0,function () {
      userApi.requestAppApi_GET(app.globalData.mainDomain + "/Home/incorrent/examresult", { id: id}, function (res) {
        ...
        var _time = parseInt((timenow - starttime) / 1000, 10) ? parseInt((timenow - starttime) / 1000, 10):0;
        if(_time < 60){
          if (_time >= 0 && _time <= 9) {
            _time = "00:0" + _time;
          }else{
        ...
        const _questions = res.data.data.questions;
        for(var j=0;j<_questions.length;j++){
          let _parent = parseInt((_questions[j].pslstats.error / _questions[j].pslstats.count) * 100);
          if (!_parent){
            _parent = 0;
          }
          _questions[j].glbstats.parent = _parent + '%';
        }
        that.setData({
