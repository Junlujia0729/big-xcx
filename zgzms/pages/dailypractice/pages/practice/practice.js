...
Page({
  ...
  onLoad: function (options) {
    ...
    userApi.xcx_login_new(1,0,function(){
      ...
      // 试卷中题目ID
      userApi.requestAppApi_GET(app.globalData.mainDomain + "Daily/job_questions", { jobid: jobid }, function (res) {
        // 提取所有题目ID
  ...
  getQuestions: function(index) {
    ...
    // 获取题
    userApi.requestAppApi_POST(app.globalData.mainDomain + "Daily/questions",
      { ids: strids.toString(), jobid: that.data.jobid },
      function (res) {
        console.log(res.data.data);
    ...
    wx.showModal({
      ...
      success: function (modres){
        if (modres.confirm){
          ...
          userApi.requestAppApi_POST(app.globalData.mainDomain + "Daily/record",
            { data: JSON.stringify(message) },
            function (res) {
              console.log(res.data);
              let analysis = that.data.examanalysis;
              analysis.examresult = res.data.data.examresult;
              for (var i = 0; i < analysis.items.length; i++) {
                analysis.items[i].globalstats = res.data.data.globalstats[i];
                analysis.items[i].personalstats = res.data.data.personalstats[i];
              }
              that.setData({
